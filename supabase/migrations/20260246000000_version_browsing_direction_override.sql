-- ============================================
-- Fix: Version browsing shows repeating tasks only on historically active dates
--
-- Problem: When browsing an archived roadmap version, get_week_tasks and
-- get_today_tasks use resolve_directions_for_date() which only returns
-- directions that were active on each specific date. Repeating tasks from
-- an archived version only appear on dates when that version was historically
-- active, not across the entire viewed week.
--
-- Solution: Add optional p_direction_id parameter to both RPCs.
-- When provided (version browsing mode), use ARRAY[p_direction_id] instead
-- of resolve_directions_for_date(), bypassing date-based resolution.
-- Also relax goal status constraint when direction override is active
-- (archived version goals may not be 'active'/'maintenance').
-- ============================================

-- 1) get_today_tasks — add p_direction_id override
    ...
    OR (t.goal_id IS NULL AND t.area_id IS NOT NULL AND EXISTS (
      ...
      AND a.direction_id = ANY(
        CASE WHEN p_direction_id IS NOT NULL
          ...
          ELSE resolve_directions_for_date(p_user_id, p_date)
        END
      )
    ))
...
-- 2) get_week_tasks — add p_direction_id override
...
LEFT JOIN LATERAL (
  SELECT
    CASE WHEN p_direction_id IS NOT NULL
      THEN ARRAY[p_direction_id]
      ELSE resolve_directions_for_date(p_user_id, d.dt::date)
    END AS dir_ids
) dir_ctx ON true
LEFT JOIN LATERAL (
  SELECT json_agg(
