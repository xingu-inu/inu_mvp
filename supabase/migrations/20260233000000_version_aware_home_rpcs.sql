-- ============================================
-- Version-Aware Home RPCs
--
-- Problem: get_today_tasks and get_week_tasks filter by
-- d.status = 'active' on the directions table. This means
-- past dates only show tasks from the CURRENT roadmap version,
-- losing historical context when a user archives a direction
-- and starts a new one.
--
-- Solution: Introduce resolve_direction_for_date() helper that
-- picks the correct direction for any given date:
--   - Today/future → current active direction
--   - Past dates   → direction that was active on that date
--     (by checking created_at/archived_at windows)
--   - Fallback     → oldest direction
--
-- Both RPCs now call this helper instead of hardcoding
-- d.status = 'active', making them version-aware.
-- ============================================

-- ============================================
-- 1) resolve_direction_for_date — helper function
-- ============================================
...
DECLARE
  v_dir_id UUID;
BEGIN
  -- For today or future: return current active direction
  ...
  -- For past dates: find direction that was active on that date
  SELECT id INTO v_dir_id
  FROM directions
...
-- ============================================
-- 2) get_today_tasks — version-aware direction filter
-- ============================================
...
-- ============================================
-- 3) get_week_tasks — version-aware direction filter
--    Uses LATERAL to resolve direction once per day
-- ============================================
