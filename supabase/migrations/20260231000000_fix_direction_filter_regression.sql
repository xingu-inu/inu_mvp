-- ============================================
-- Fix: Restore direction filter in get_today_tasks / get_week_tasks
--
-- Migration 20260224 (add_task_area_id) overwrote the direction filter
-- that 20260222 (roadmap_versioning) had added. This caused tasks from
-- archived roadmap versions to appear on the Home screen.
--
-- This migration restores the direction filter while keeping all fields
-- from 20260224 (areaId, directArea, etc.) intact.
-- ============================================

-- ============================================
-- 1) get_today_tasks — restore direction filter
-- ============================================
...
WHERE t.user_id = p_user_id
  AND (
    ...
    -- Goalless + has area → only if area belongs to active direction
    OR (t.goal_id IS NULL AND t.area_id IS NOT NULL AND EXISTS (
      SELECT 1 FROM areas a
      JOIN directions d ON d.id = a.direction_id
      WHERE a.id = t.area_id AND d.status = 'active'
    ))
    -- Goal-based → goal active/maintenance AND in active direction
    OR EXISTS (
...
-- ============================================
-- 2) get_week_tasks — restore direction filter
-- ============================================
