...
// AI 코치 대화 내역 저장/조회
//
// NOTE: chat_conversations, chat_messages 테이블은 새 migration으로 추가.
// `npm run db:types` 실행 전까지 supabase.from()에 타입이 없어서
// eslint-disable를 사용합니다. db:types 실행 후 제거할 수 있습니다.
...
// db:types 실행 전까지 타입 미지원 테이블 접근용
function from(supabase: TypedSupabaseClient, table: string) {
...
export const chatRepository = {
  ...
  async getMessages(supabase: TypedSupabaseClient, conversationId: string): Promise<ChatMessage[]> {
    const { data, error } = await from(supabase, 'chat_messages')
      .select('*')
  ...
  async addMessage(
    ...
    if (role === 'user') {
      await from(supabase, 'chat_conversations')
        .update({
  ...
  async deleteConversation(
    ...
    const { error } = await from(supabase, 'chat_conversations')
      .delete()
      .eq('id', conversationId)
      .eq('user_id', userId)
