...
import type {
  ...
  BrainDumpReviewGoal,
  BrainDumpReviewTask,
} from '../../hooks/use-brain-dump-apply'

interface BrainDumpReviewStepProps {
  areas: BrainDumpReviewArea[]
  onAreasChange: (areas: BrainDumpReviewArea[]) => void
  onApply: () => void
  onRetry: () => void
  summary: string
}

export function BrainDumpReviewStep({
  areas,
  onAreasChange,
  onApply,
  onRetry,
  summary,
}: BrainDumpReviewStepProps) {
  ...
  function updateGoalField(
    areaId: string,
    goalId: string,
    field: keyof BrainDumpReviewGoal,
    value: string
  ) {
    onAreasChange(
      areas.map((a) =>
        a._id === areaId
          ? { ...a, goals: a.goals.map((g) => (g._id === goalId ? { ...g, [field]: value } : g)) }
          : a
      )
    )
  }
  ...
  function updateTaskField(
    ...
    goalId: string,
    taskId: string,
    field: keyof BrainDumpReviewTask,
    value: string | number
  ) {
    onAreasChange(
      areas.map((a) =>
          ...
          ? {
              ...
              goals: a.goals.map((g) =>
                g._id === goalId
                  ? {
                      ...g,
                      tasks: g.tasks.map((t) => (t._id === taskId ? { ...t, [field]: value } : t)),
                    }
                  : g
              ),
            }
  ...
  return (
    <>
                      ...
                      {area.goals.map((goal) => (
                        ...
                        >
                          ...
                          <div className="flex items-center gap-2 px-3 py-2 pl-9">
                            ...
                            {editingId === goal._id ? (
                              <input
                                ...
                                value={goal.name}
                                onChange={(e) =>
                                  updateGoalField(area._id, goal._id, 'name', e.target.value)
                                }
                              ...
                              <>
                                ...
                                <button
                                  ...
                                  className="flex-shrink-0 rounded p-1 text-[var(--color-text-quaternary)] opacity-0 transition-opacity group-hover/goal:opacity-100 hover:bg-[var(--color-bg-tertiary)] hover:text-[var(--color-text-secondary)] [div:hover>&]:opacity-100"
                                >
                          ...
                          {goal.tasks.map((task) => (
                            <div
                              key={task._id}
                              className="flex items-center gap-2 px-3 py-1.5 pl-14"
                            >
                              <Checkbox
                              ...
                              {editingId === task._id ? (
                                ...
                                <>
                                  ...
                                  <button
                                    ...
                                    className="flex-shrink-0 rounded p-1 text-[var(--color-text-quaternary)] opacity-0 transition-opacity hover:bg-[var(--color-bg-tertiary)] hover:text-[var(--color-text-secondary)] [div:hover>&]:opacity-100"
                                  >
                                    <Pencil className="h-3 w-3" />
                                  </button>
                                </>
      ...
      <div className="mt-6 flex gap-3">
        ...
        <Button onClick={onApply} disabled={checkedCount === 0} className="flex-1 gap-1.5">
          <Check className="h-4 w-4" />
          {checkedCount}개 항목 추가
        </Button>
      </div>
    </>
  )
