...
export function createChatTools(supabase: TypedSupabaseClient, userId: string) {
  return {
    ...
    suggest_responses: tool({
      description:
        '대화의 다음 단계로 사용자가 선택할 수 있는 응답 옵션을 제안합니다. 모든 응답 끝에 호출하세요. multi=true면 멀티 선택 + 합쳐서 보내기.',
      inputSchema: z.object({
        chips: z
          .array(
            z.object({
        ...
        multi: z
          ...
          .optional()
          .describe(
            'true면 사용자가 여러 칩을 합쳐서 보낼 수 있음. 사용자가 여러 주제를 동시에 꺼낼 법할 때만 true. 결정/전환 칩은 false.'
          ),
      }),
      execute: async ({ chips, multi }) => {
    ...
    propose_structure: tool({
      description:
        '사용자의 아이디어나 계획을 Area/Goal/Task 구조로 제안합니다. 사용자가 하고 싶은 것을 언급하면 즉시 호출하세요. 완벽하지 않아도 됩니다 — 대화의 출발점입니다. 사용자는 제안을 미리보고 "반영하기"로 실제 생성할 수 있습니다.',
      inputSchema: z.object({
